...
RSpec.describe QueryController, type: :controller do
  ...
  describe "GET #search" do
    ...
    context 'with correct params for Question' do
      ...
      it 'status 2xx' do
        expect(ThinkingSphinx).to receive(:search).with("Title title", page: '0', classes: [Question], per_page: 15).and_return([question])
        get :search, params: params, xhr: true, format: :js
        expect(response).to be_successful
      end

      it 'renders template search' do
        allow(ThinkingSphinx).to receive(:search).with("Title title", page: '0', classes: [Question], per_page: 15).and_return([question])
        get :search, params: params, xhr: true, format: :js
    ...
    context 'with correct params for Answer' do
      ...
      it 'status 2xx' do
        expect(ThinkingSphinx).to receive(:search).with("Answer body", page: '0', classes: [Answer], per_page: 15).and_return([answer])
        get :search, params: params, xhr: true, format: :js
        expect(response).to be_successful
      end

      it 'renders template search' do
        allow(ThinkingSphinx).to receive(:search).with("Answer body", page: '0', classes: [Answer], per_page: 15).and_return([answer])
        get :search, params: params, xhr: true, format: :js
        expect(response).to render_template :search 
      end
    end
    ...
    context 'with correct params for Comment' do
      ...
      it 'status 2xx' do
        expect(ThinkingSphinx).to receive(:search).with("Comment body", page: '0', classes: [Comment], per_page: 15).and_return([comment])
        get :search, params: params, xhr: true, format: :js
        expect(response).to be_successful
      end

      it 'renders template search' do
        allow(ThinkingSphinx).to receive(:search).with("Comment body", page: '0', classes: [Comment], per_page: 15).and_return([comment])
        get :search, params: params, xhr: true, format: :js
        expect(response).to render_template :search 
      end
    end
    ...
    context 'with correct params for User' do
      ...
      it 'status 2xx' do
        expect(ThinkingSphinx).to receive(:search).with(user.email, page: '0', classes: [User], per_page: 15).and_return([user])
        get :search, params: params, xhr: true, format: :js
        expect(response).to be_successful
      end

      it 'renders template search' do
        allow(ThinkingSphinx).to receive(:search).with(user.email, page: '0', classes: [User], per_page: 15).and_return([user])
        get :search, params: params, xhr: true, format: :js
        expect(response).to render_template :search 
      end
    end
    ...
    context 'with correct params for all classes' do
      ...
      it 'status 2xx' do
        expect(ThinkingSphinx).to receive(:search).with('ruby', page: '0', classes: [Question, Answer, Comment, User], per_page: 15).and_return([user, answer, question, comment])
        get :search, params: params, xhr: true, format: :js
        expect(response).to be_successful
      end

      it 'renders template search' do
        allow(ThinkingSphinx).to receive(:search).with('ruby', page: '0', classes: [Question, Answer, Comment, User], per_page: 15).and_return([user, answer, question, comment])
        get :search, params: params, xhr: true, format: :js
        expect(response).to render_template :search 
      end
    end
